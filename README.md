...
Worldgen terrain planning uses deterministic four-block interpolated surface and solid-support height grids plus noise biomes instead of reading neighboring chunks. A surface above its motion-blocking support height is treated as fluid terrain independently of biome id, so swamp water and inland ponds follow the current profile's water policy. This is intentionally lighter and less detailed than the loaded-world debug survey. Final placement resolves each operation against solid support in its own chunk, fills approved gaps, and clears vanilla logs or leaves above affected columns before placing roads and placeholder buildings.

Road segments and building slots carry deterministic semantic platform elevations. Roads use the median terrain height of each complete segment footprint, while buildings use the highest terrain height inside their footprint so uphill ground cannot bury part of a structure. Worldgen cuts terrain above the selected elevation and fills lower columns with the vanilla foundation material before placement. Fill depth and total earthwork budgets reject unsupported pads instead of creating unbounded foundations over ravines. Each building pad and each road segment is therefore flat even when it crosses chunk boundaries. Separate connected road segments may use different elevations; smooth transitions, stairs, retaining walls, styled foundations, tunnels, and switchbacks remain future terrain-aware work.
