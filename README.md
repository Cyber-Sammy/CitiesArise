...
Road segments and building slots carry deterministic semantic platform elevations. Roads use the median terrain height of each complete segment footprint, while buildings use the highest terrain height inside their footprint so uphill ground cannot bury part of a structure. Worldgen cuts terrain above the selected elevation and fills lower columns with the vanilla foundation material before placement. Building pads also receive bounded three-block earth-and-grass shoulders on supported downhill terrain, while a dedicated foundation-depth limit rejects sites that would still require tall exposed supports. Shoulder columns are checked for water and blocked terrain and count toward the same earthwork budget before a settlement is accepted. Each building pad and each road segment is therefore flat even when it crosses chunk boundaries. The regional elevation plan keeps these flat zones and their road or building-access transitions as one authoritative model. It deliberately permits different parts of a settlement to occupy different levels instead of flattening the whole city onto one plane. Smooth transition geometry, stairs, retaining walls, styled foundations, tunnels, and switchbacks remain future terrain-aware work.
...
Successful debug summaries report `terrain=ACCEPTED` when no cut or fill is required and `terrain=ACCEPTED_WITH_EARTHWORKS` with the calculated cut and fill volumes when preparation is required. Accepted plans also report `earthworkQuality`: `DIRECT` requires no cut or fill, `MODERATE` stays inside preferred depths, and `MAJOR` remains valid but exceeds at least one preferred depth. The JSON debug dump includes the same assessment inputs. Rejected cut, fill, or total-volume diagnostics include the responsible plan element, actual value, preferred limit, absolute limit, and excess. `/citiesarise locate` treats all successful qualities as valid and ranks them without weakening hard safety limits.
